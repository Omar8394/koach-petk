...
class ConfTablasConfiguracion(models.Model):
    ...
    def obtenerHijos(valor):
        ...
        try:
            ...
            padre = ConfTablasConfiguracion.objects.filter(valor_elemento=valor)
           
            lista = ConfTablasConfiguracion.objects.filter(fk_tabla_padre=padre[0].id_tabla)
            return lista
...
class ConfSettings(models.Model):
    ...
    fk_modulo_setting = models.ForeignKey(ConfTablasConfiguracion, on_delete=models.DO_NOTHING, db_column='fk_modulo_setting', null=True) 
